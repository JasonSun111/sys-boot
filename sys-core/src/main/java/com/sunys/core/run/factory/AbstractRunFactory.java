...
public abstract class AbstractRunFactory implements RunFactory {

	public abstract Run createRun();
	
	@Override
	public Run getInstance() throws Exception {
		Run run = createRun();
		Run runProxy = getProxy(run);
		return runProxy;
	}

	private Run getProxy(Run run) throws Exception {
